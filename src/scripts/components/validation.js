function showInputError(inputElement, errorMessage, settings) {
  ...
  function checkInputValidity(inputElement, settings) {
    if (inputElement.validity.valueMissing) {
      showInputError(inputElement, 'Вы пропустили это поле', settings);
    } else if (inputElement.validity.tooShort) {
      showInputError(inputElement, `Минимальное количество символов: 2. Длина текста сейчас: ${inputElement.value.length}`, settings);
      ...
      showInputError(inputElement, 'Введите корректный URL', settings);
    } else {
